...
async function sendBibleVerses({ verse, translation, say, client, channel, messageTs }) {
  ...
  try {
    ...
    const title = `*${first.book_name} ${first.chapter}:${verseRange} (${translation.toUpperCase()})*`;
    const versesText = data.verses.map(v => `*${v.verse}*: ${v.text.trim()}`).join('\n');
    ...
    if (client && channel && messageTs) {
      await client.chat.update({ channel, ts: messageTs, blocks, text: `${title}\n${versesText}` });
    } else {
...
translations.forEach(t => {
  app.action(`bible_translation_${t.abbreviation}`, async ({ body, ack, client }) => {
    await ack();
